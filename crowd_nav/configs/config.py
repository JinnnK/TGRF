...
class Config(object):
    ...
    reward = BaseConfig()
    reward.success_reward = 10   # you can change this, but hyperparameters below are more important.
    reward.collision_penalty = -10   # you can change this, but hyperparameters below are more important.
    reward.gamma = 0.99

    # Hyperparameters
    """
    Choose danger zone type among 'circle', 'future', and 'gaussian'.
    Circle has no discomfort distance.
    Future has discomfort distance and calculate reward discretely depending on how close between human and robot.

    Gaussian is my new method.
    Gaussian also has discomfort distance but it gives reward continuously depending the distance between human and robot.
    And human has different continuous discomfort distance which changes everytime depending on the velocity of the human.
    Ahead range of discomfort distance has larger range than the behind.
    """
    reward.danger_zone_type = 'gaussian'
    reward.discomfort_dist = 0.5
    reward.sigma = 0.2  # control the Standard Deviation
    reward.penalty_intensity = 0.25 # control the Intensity of penalty
    reward.goal_factor = 1.5   # control the reward when the robot get closer to the goal

    reward.penalty_intensity_pot = 1.5
    reward.sigma_pot = 1000
