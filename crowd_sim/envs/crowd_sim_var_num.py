...
class CrowdSimVarNum(CrowdSim):
    ...
    def configure(self, config):
        ...
        self.normalized_factor = sc.norm(0, config.reward.sigma).pdf(0)
        self.normalized_factor_pot = sc.norm(0, config.reward.sigma_pot).pdf(0)
        self.danger_zone_type = config.reward.danger_zone_type
        self.penalty_intensity_pot = config.reward.penalty_intensity_pot
        self.sigma_pot = config.reward.sigma_pot
    ...
    def step(self, action, update=True):
        ...
        reward, done, episode_info = self.calc_reward(action, danger_zone=self.danger_zone_type)
    ...
    def calc_reward(self, action, danger_zone):
        # collision detection
        dmin = float('inf')
        ...
        for i, human in enumerate(self.humans):
            ...
            # closest_dist is the closest distance between human and robot
            closest_dist = (dx ** 2 + dy ** 2) ** (1 / 2) - human.radius - self.robot.radius

            # in the case of closest distance is smaller than discomfort distance
            if closest_dist < self.discomfort_dist:
                danger_dists.append(closest_dist)
            # in the case of closest distance is smaller than 0
            if closest_dist < 0:
                collision = True
                break
            # in the case the other cases
        ...
        if self.global_time >= self.time_limit - 1:
            ...
            # check which position they collide with
        ...
        elif danger_cond and danger_zone == 'gaussian':
            # only penalize agent for getting too close if it's visible
            # adjust the reward based on FPS
            # dmin is the distance between robot and human when it is in the discomfort distance.
            reward = sc.norm(0, self.sigma).pdf(dmin) / self.normalized_factor * self.collision_penalty * self.penalty_intensity
            done = False
            ...
            # adjust the reward based on FPS
            # dmin is the distance between robot and human when it is in the discomfort distance.
            reward = 10 * (dmin - self.discomfort_dist) * self.time_step
            done = False
            episode_info = Danger(min_danger_dist)
            ...
            # potential reward
            if self.robot.kinematics != 'holonomic':
                self.goal_factor = self.goal_factor * 1.5
            potential_cur = np.linalg.norm(
                np.array([self.robot.px, self.robot.py]) - np.array(self.robot.get_goal_position()))
            reward = sc.norm(0, self.sigma_pot).pdf(0) / self.normalized_factor_pot * self.penalty_intensity_pot * (-abs(potential_cur) - self.potential)
